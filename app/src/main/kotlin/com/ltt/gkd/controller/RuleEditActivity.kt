...
import com.ltt.gkd.model.util.DeviceIdProvider
import com.ltt.gkd.model.util.Logger
import com.ltt.gkd.view.rule.RuleEditScreen
...
class RuleEditActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 编辑模式：从已加载规则中找；同步阻塞 reload 保证非空启动
        val initial: Rule = if (initialId.isNotEmpty()) {
            kotlinx.coroutines.runBlocking { repo.reload() }
            repo.rules.value.firstOrNull { it.id == initialId } ?: Rule(id = initialId, name = "")
    ...
    private suspend fun uploadRule(rule: Rule): Boolean = withContext(Dispatchers.IO) {
        ...
        val ruleSet = com.ltt.gkd.model.rule.RuleSet(
...
/** 用 App 全局协程作用域启动，避免在 Controller 层直接依赖 lifecycle-runtime-ktx。 */
