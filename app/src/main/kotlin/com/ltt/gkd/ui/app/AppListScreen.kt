...
import com.ltt.gkd.data.app.AppInfo
...
fun AppListScreen(repo: AppListRepository) {
    ...
    // 关键字变化时去抖 250ms 后再搜索，避免每次按键都过滤
    LaunchedEffect(keyword) {
        // 等待 250ms，若期间关键字又变化则本协程被取消，新值会启动新协程
        kotlinx.coroutines.delay(250)
        val data = if (keyword.isBlank()) {
            // 空关键字：用全量（命中缓存）
            withContext(Dispatchers.IO) { repo.listAll() }
        } else {
            withContext(Dispatchers.IO) { repo.search(keyword) }
        }
        allApps.value = data
    }
